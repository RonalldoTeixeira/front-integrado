...
import { Div, DivInteracoes, DivLikeDislike, DivMessage, P } from "./style"
...
const LayoutPost = () => {
    ...
    const { token, post, setSearchPost, setComments } = context

    const getPost = (post) => {
        axios.get(
            `https://labeddit-backend-slzf.onrender.com/posts?q=${post.id}`,
            {
                headers: {
                    Authorization: token
                }
    ...
    const getComments = (post) => {
        axios.get(
            `https://labeddit-backend-slzf.onrender.com/comments?q=${post.id}`,
            {
                headers: {
                    Authorization: token
                }
